...
suite("Extension Test Suite", () => {
  ...
  suite("getAttributeName", () => {
    test("when attribute name is correct", () => {
      ...
      const marginLeftAttribute =
        myExtension.getAttributeName(marginLeftPrefix);

      assert.strictEqual(marginLeftAttribute, "marginLeft");

      const colorPrefix =
        '<div marginTop=`space20` marginLeft=`space40` color="';
      ...
      const fontSizeAttribute =
        myExtension.getAttributeName(fontSizeWithoutQuote);
